...
class serverInst(object):
    """docstring for serverInst"""
    def __init__(self, app, host, port, reloader):
        super(serverInst, self).__init__()
        self.app = app
        self.host = host
        self.port = port
        self.reloader = reloader
        run(app = self.app, host = self.host, port = self.port,reloader = self.reloader)

for i in range(1,11):
    serverInst(app,'0.0.0.0',(8080+i),True)
